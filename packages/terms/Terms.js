...
/*
  ...
  Holds all terms.
*/
class Terms extends Document {
  constructor(schema) {
    super(schema)

    this.addIndex('terms', new TermsIndex(this))
  }

  // Get children nodes for a given node
  getChildren(nodeId) {
    let index = this.getIndex('terms').index
  ...
  getActive() {
    let active = []
    let termNodes = this.getIndex('type').get('term')
    each(termNodes, function(node) {
